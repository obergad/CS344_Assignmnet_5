...
* 2. Promci the user for input and send that input as a message to the server.
...
char *getMessage(char* key, char* ciphertext){
  ...
  char *tempStr_ci = calloc(MAXCHAR, sizeof(char));
  ...
  strcpy(myString, "dec \n");
  myFile = fopen(ciphertext, "r"); //open ciphertext file and put
  ...
  strcat(myString, tempStr_ci); //cat ciphertext into string
